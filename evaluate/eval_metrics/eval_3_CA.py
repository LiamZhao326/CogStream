...
- 0-1分：完全忽略历史上下文，提供不相关的信息
- 2-3分：涵盖很少的上下文，遗漏了关键元素或引入了不相关/错误的内容
- 4-6分：大致覆盖必要的上下文，但包括一些冗余或不相关的细节
- 7-8分：提到关键的上下文元素，并能适当地使用它们
- 9-10分：充分整合并利用所有相关的上下文信息，有效地回答问题
...
def annotate(seq_data,file,seq_id):
    ...
    for qa_id, single_qa in enumerate(seq_data):
        ...
        prompt1 = '''
You are a video QA evaluation expert specializing in analyzing the **Context Awareness** of generated answers. 
...
- 0-1: Completely ignores historical context, providing unrelated information.
- 2-3: Mentions minimal context but misses key elements or introduces irrelevant/incorrect content.
- 4-6: Roughly covers necessary context but includes some redundancy or irrelevant details.
- 7-8: Mentions key context elements and uses them appropriately.
        ...
        while True:
            try:
                if (response := client.chat(messages)) is not None:
                    print(response)
                    output = extract_scores(response,qa_id)
...
def parse_args():
    ...
    parser.add_argument("--input_root", type=str, required=False,default='/mnt/nvme0n1/liamz/StreamVICL-main/streamvicl/eval/Streamvicl/input', #  Streamvicl/qa_dataset
                        help="包含QA数据的JSON目录")
    parser.add_argument("--output_root", type=str, default="/mnt/nvme0n1/liamz/StreamVICL-main/streamvicl/eval/Streamvicl/CA",
                        help="输出结果目录")
