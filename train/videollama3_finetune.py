...
def forward_baseline(conversation, model, processor, answer, accelerator):
    inputs = processor(
        conversation=conversation,
        add_system_prompt=True,
        add_generation_prompt=True,
    ...
    answer_tokens = processor.tokenizer(answer, return_tensors='pt', add_special_tokens=False).input_ids.to(accelerator.device)
            ...
            for video_id, batch in enumerate(effective_dataloader):
                ...
                for current_query_time, file_name in zip(sorted_times, natural_sort_segments(video_path)):
                    ...
                    for i, qa in enumerate(segments[current_query_time]):
                        ...
                        try:
                            loss = forward_baseline(conversation, model, processor, answer, accelerator)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_path", type=str, help="Path to the base model directory.", default="/mnt/nvme0n1/liamz/StreamVICL-main/streamvicl/eval/baseline_models/VideoLLaMA3/VideoLLaMA3-7B")
    parser.add_argument("--video_dir", type=str, help="Directory containing train video files.", default="/mnt/sda/Streamvicl/Train_dataset/event_segments")
    parser.add_argument("--query_dir", type=str, help="Directory containing train query (QA) files.", default="/mnt/sda/Streamvicl/Train_dataset/QA_Dataset")
    parser.add_argument("--num_epochs", type=str, help="Training epochs.", default=3)
    ...
    lora_save_dir = "./videollama3_fine_tuning"
