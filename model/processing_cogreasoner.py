"""Processor class for CogReasoner."""
...
class Videollama3Qwen2Processor(ProcessorMixin):
    ...
    def load_video(
            ...
            Args:
                ...
                max_frames (float, optional): Number of frames to sample. Defaults to None.
                ...
                trim_time (float, optional): Time in seconds to trim from the start. Defaults to 0.0.
            ...
            if trim_time > 0:
                stream = ffmpeg.trim(stream, start=trim_time, end=math.floor(duration-trim_time)+trim_time)
    ...
    def _load_multimodal_data(self, conversation: Conversation):
        ...
        for data_id, contents in multimodal_info.items():
            ...
            if data_type == "image":
                ...
                fractional_part, _ = math.modf(total_duration)
                if total_duration == 0:
                    fractional_part = 1
                images, timestamps, duration = self.load_video(trim_time=1-fractional_part, **load_args)
                ...
                for content, start_time, end_time in zip(contents, start_times, end_times):
                    ...
                    for image, timestamp in zip(images, timestamps):
                        if start_time <= timestamp <= end_time:
                            ...
                            cur_timestamps.append(timestamp+seg_time_end)
                            all_timestamps.append(timestamp+seg_time_end)
                            max_time = timestamp+seg_time_end

                    content[data_type] = cur_images
                    content["num_frames"] = len(cur_images)
                    content["timestamps"] = cur_timestamps
                    # print(cur_timestamps)
                    seg_time_end = max_time + 1  

        return new_conversation, all_timestamps
    ...
    def _process_conversation(
        ...
        **kwargs: Unpack[Videollama3Qwen2ProcessorKwargs],
        ...
        if images is None:
            conversation, all_timestamps = self._load_multimodal_data(conversation)
        ...
        if return_labels:
            ...
            text_inputs, text = self._process_conversation_without_label(conversation, image_inputs, **kwargs)

        total_image_num = 0
        for image in images:
            total_image_num += len(image[1])

        hist_qs, hist_as, current_question = self.process_history_qas(conversation)
        return BatchFeature(data={**text_inputs, **image_inputs, "tokenizer": self.tokenizer, "hist_qs": hist_qs, "hist_as": hist_as, "current_question": current_question, "all_timestamps": all_timestamps, "total_image_num": total_image_num, "original_text": text})
    ...
    def process_history_qas(self, conversation):
        hist_qs = []
        hist_as = []
        ...
        for item in conversation:
            ...
            if role == "user":
                if isinstance(content, str):
                    hist_qs.append(content)
                elif isinstance(content, list):
                    for sub_item in content:
                        if isinstance(sub_item, dict) and sub_item.get("type") == "text":
                            hist_qs.append(sub_item.get("text"))
            elif role == "assistant":
                hist_as.append(content)
        
        current_question = hist_qs[-1] if hist_qs else ""
        hist_qs = hist_qs[:-1]
        return hist_qs, hist_as, current_question
